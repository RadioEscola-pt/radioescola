class ResonanceCalculator extends ElectricalUnits{
    ...
    endRequest() {
        const calculateButton = document.getElementById("ReesunaceCalculatorCalculateButton");
        calculateButton.addEventListener("click", this.calculateResonance.bind(this));
    ...
    calculateResonance() {
        ...
        if (!isNaN(inductance) && !isNaN(capacitance)) {
            let resonanceFrequency = 1 / (2 * Math.PI * Math.sqrt(inductance*unitInduValue * capacitance*unitCapValue));
           
            document.getElementById("result").textContent = `A frequência de ressonância é ${this.formatNumberWithExponent(resonanceFrequency)} Hz.`;
        } else {
