class SimulateQuiz extends Classes([Questions, Storage]) {
  ...
  checkTimer(simulateQuiz) {
    ...
    counterDiv.innerHTML = new Date(simulateQuiz.timeout * 1000)
      .toISOString()
      .slice(11, 19);
  ...
  showPage() {
    ...
    if (document.querySelector("#qIndex button.active")) {
      document
        .querySelector("#qIndex button.active")
  ...
  checkQuestions() {
    ...
    for (var questionIndex in this.questions) {
      ...
      for (var i = 0; i < elements.length; i++) {
        if (elements[i].checked) {
          const correctIndex = parseInt(
            this.questions[questionIndex].correctIndex,
          );
          if (correctIndex == i + 1) {
      ...
      if (answers[questionIndex] != SimulateQuiz.CORRECT) {
        noteDiv.innerHTML =
          "Resposta certa: " + this.questions[questionIndex].correctIndex;
        noteDiv.className = "incorrect";
        noteDiv.innerHTML += "<br>" + this.questions[questionIndex].notes;

        noteDiv.parentElement.parentElement.parentElement.style.boxShadow =
          "0px 0px 5px #7F1D1D";
        elements[
          parseInt(this.questions[questionIndex].correctIndex - 1)
        ].parentElement.style.color = "#22C55E";
      }
    }
    var total = 0;
    ...
    counterDiv.innerHTML =
      "Pontos:" + total + " em 40  a classificação mínima de 20 pontos";
  }
      ...
      if (ajaxRequest.readyState == 4) {
        ...
        if (ajaxRequest.status == 200) {
          ...
          for (let i = 0; i < SimulateQuiz.QUESTION_AMOUNT; i++) {
            var questionIndex = Math.floor(
              Math.random() * allMessagesArray.length,
            );
          ...
          for (var qindex in randomQ) {
            ...
            if (questionCounter == 0) {
              ...
              pageBlock.dataset.page =
                questionCounter / SimulateQuiz.PER_PAGE + 1;
            ...
            questiontxt.innerHTML =
              index + ") " + this.simulateQuiz.questions[qindex].question;
            ...
            if (this.simulateQuiz.questions[qindex].correctIndex == null) {
              console.log(
                "ERROR Q" + this.simulateQuiz.questions[qindex].uniqueID,
              );
            }
          ...
          this.simulateQuiz.timer = window.setInterval(
            this.simulateQuiz.checkTimer,
            1000,
            this.simulateQuiz,
          );
        } else {
