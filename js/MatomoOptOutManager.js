class MatomoOptOutManager {
  settings = {
    ...
    OptOutComplete:
      "As suas visitas a este website não serão registadas pela ferramenta de Análise Web.",
    OptOutCompleteNextLine:
      "Note que se limpar os seus cookies, eliminar o cookie de opt-out ou se mudar de computador ou navegador web, terá de realizar novamente o procedimento de opt-out.",
    YouMayOptOutPart1:
      "Pode optar por impedir este website de agregar e analisar as ações que realiza aqui.",
    YouMayOptOutPart2:
      "Fazê-lo protegerá a sua privacidade, mas também impedirá o proprietário de aprender com as suas ações e criar uma experiência melhor para si e outros utilizadores.",
    OptOutErrorNoCookies:
      "A funcionalidade de rastreamento requer que os cookies estejam ativados.",
    OptOutErrorNotHttps:
      "A funcionalidade de rastreamento pode não funcionar porque este site não foi carregado via HTTPS. Por favor, recarregue a página para verificar se o seu estado de opt-out foi alterado.",
    ...
    OptOutErrorNoTracker:
      "A funcionalidade de  rastreamento não conseguiu encontrar o código Matomo Tracker nesta página.",
  };
  constructor() {
  ...
  checkForMatomoTracker() {
    ...
    console.log(
      "Matomo OptOutJS: failed to find Matomo tracker after " +
        (this.checkForTrackerTries * this.checkForTrackerInterval) / 1000 +
        " seconds",
    );
  }
  ...
  consentGiven() {
    MatomoOptOutManager.setCookie(
      MatomoOptOutManager.CONSENT_REMOVED_COOKIE_NAME,
      "",
      -129600000,
    );
    MatomoOptOutManager.setCookie(
      MatomoOptOutManager.CONSENT_COOKIE_NAME,
    ...
    if (consent) {
      ...
      if (this.settings.showIntro) {
        const introText = document.createTextNode(
          this.settings.YouMayOptOutPart1 +
            " " +
            this.settings.YouMayOptOutPart2,
        );
      ...
      labelText = document.createTextNode(
        this.settings.YouAreNotOptedOut + " " + this.settings.UncheckToOptOut,
      );
    } else {
      ...
      if (this.settings.showIntro) {
        const introText = document.createTextNode(
          this.settings.OptOutComplete +
            " " +
            this.settings.OptOutCompleteNextLine,
        );
        introParagraph.appendChild(introText);
        div.appendChild(introParagraph);
      }
      ...
      labelText = document.createTextNode(
        this.settings.YouAreOptedOut + " " + this.settings.CheckToOptIn,
      );
    }
  ...
  static initMatomoConsentManager(
    useSecureCookies,
    cookiePath,
    cookieDomain,
    cookieSameSite,
  ...
  static hasConsent() {
    const consentCookie = MatomoOptOutManager.getCookie(
      MatomoOptOutManager.CONSENT_COOKIE_NAME,
    );
    const removedCookie = MatomoOptOutManager.getCookie(
      MatomoOptOutManager.CONSENT_REMOVED_COOKIE_NAME,
    );
  ...
  static setCookie(cookieName, value, msToExpire) {
    ...
    document.cookie =
      ...
      ";path=" +
      (MatomoOptOutManager.cookiePath || "/") +
      (MatomoOptOutManager.cookieDomain
        ? ";domain=" + MatomoOptOutManager.cookieDomain
        : "") +
      (MatomoOptOutManager.cookieIsSecure ? ";secure" : "") +
      ";SameSite=" +
      MatomoOptOutManager.cookieSameSite;
    if (
      (!msToExpire || msToExpire >= 0) &&
      MatomoOptOutManager.getCookie(cookieName) !== String(value)
    ) {
      console.log(
